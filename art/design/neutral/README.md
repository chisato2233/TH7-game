...
Prefab 是 Unity 中预制的游戏对象，包含动画、特效、碰撞体等。

**自动查找规则：**

导入时系统会自动查找 Prefab，无需在 YAML 中配置路径：
1. 先查找默认路径：`Assets/prefabs/game/world/heros/{faction}/{hero_id}/{hero_id}.prefab`
2. 找不到则使用基础 Prefab：`Assets/prefabs/game/world/heros/base_hero.prefab`

**目录结构：**
```
Assets/prefabs/game/world/
├── heros/
│   ├── base_hero.prefab          # 默认英雄 Prefab
│   ├── neutral/                  # 按文明分文件夹
│   │   └── {hero_id}/
│   │       └── {hero_id}.prefab
│   └── northsea/
...
│           └── {hero_id}.prefab
└── units/
    ├── base_unit.prefab          # 默认兵种 Prefab
    └── {faction}/
        └── {unit_id}/
            └── {unit_id}.prefab
```

**注意：** 只有需要自定义动画/特效的角色才需要创建专属 Prefab，其他角色会自动使用 base_hero/base_unit。
...
heroId: egyptian_desert_knight   # 全局唯一ID [a-zA-Z0-9_]
displayName: Desert Knight
description: A noble warrior from the scorching sands
faction: Egyptian               # [枚举] 见上方 "阵营类型" 表格
...
# 初始军队 (强引用，可选)
startingArmy:
  - unit: "@UnitConfig_egyptian_swordsman"
    count: 15
  - unit: "@UnitConfig_egyptian_archer"
    count: 8
```
...
unitId: egyptian_swordsman       # 全局唯一ID [a-zA-Z0-9_]
...
buildingId: egyptian_tavern      # 全局唯一ID [a-zA-Z0-9_]
...
goldPerDay: 0
producedUnitId: egyptian_swordsman  # 对应兵种的 unitId
weeklyGrowth: 14
```
